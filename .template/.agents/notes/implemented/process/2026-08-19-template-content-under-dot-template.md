...
- Move the entire template content into a `.template/` subdirectory of the repository: `AGENTS.md`, the bilingual `README` triplet, `docs/`, `scripts/`, `.agents/`, `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `tsconfig.json`, and `tsconfig.host.json`.
- Keep the Git root at the parent: `.git/`, `.gitattributes`, and `.gitignore` stay at the repository root, so the repository remains a container that can host the template and anything else. The template remains self-contained inside `.template/`: its scripts resolve the root via `import.meta.dirname`, its gates run with `pnpm` inside the directory, and its pairing records are content-hash based and unaffected by the move.
...
- The repository root now holds the Git metadata, the entry `AGENTS.md`, and `.template/`; `.pnpm-store/` remains at the root as a shared pnpm store cache and stays ignored.
